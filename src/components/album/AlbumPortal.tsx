...
function closedSize(width: number): [number, number] {
  if (width >= BREAKPOINTS.lg) return [190, 246];
  if (width >= BREAKPOINTS.md) return [168, 218];
...
function openSize(width: number, height: number): [number, number] {
  ...
  // Anclado a la esquina: crece hacia la izquierda y hacia arriba sin llegar
  // a la columna de la flor, que ocupa el 54 % izquierdo.
  return [Math.min(width * 0.36, 700), Math.min(height * 0.66, 580)];
}
